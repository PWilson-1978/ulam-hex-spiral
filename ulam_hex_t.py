...
def isPrimeAlt(value):
    ...
    if value % 6 in (1,5):
        ...
        if testval > 2 or value > 1:
             if value in prime_vals:
                 return True
             else:
                 plen = len(prime_vals)
                 for i in range(0,plen):
                      listval = int(prime_vals[i])
                      if listval <= value:
                          if value % listval == 0:
                              return False
                          if prime_vals[-1] == listval and listval + 6 < value:
                              rval = mini_isPrime(testval, value)
                              return rval
    ...
    return False
###################
def mini_isPrime(testval, value):
